...
loses invalidations. It costs 1{,}009
...
Table~\ref{tab:fpga} gives the breakdown. The filter costs 1{,}009 LUTs and
519 flip-flops over the unfiltered design, under 4\% of it, and no block RAM or
DSP at all. Both configurations meet every constraint, at $+4.307$~ns and
$+2.597$~ns of slack with no failing endpoints.

Two things in the breakdown are worth drawing out. Half of the filter's 414
...
\textbf{Directed invalidation}, which is the obvious next step and does not pay.
The mirror knows not merely \emph{whether} another cache holds the line but
\emph{which} ones do, so the bus can drive the invalidation into exactly those
caches and hold the grant on only those: an exact multicast with a partial
barrier. Only an exact filter may do this -- a conservative one may over-claim
sharers safely under suppression, but under multicast a missing sharer bit loses
an invalidation instead of costing a redundant broadcast. We built it and
measured it on a halo-exchange stencil, a Jacobi 5-point kernel split into row
bands so that each band boundary is written by one PE and read by its
neighbour, giving a sharer set of exactly one of the three other caches. It
behaves perfectly: 168 invalidations delivered, all 168 useful, against
suppression's 504, and the residual snoop stall falls elevenfold. It is also
worth 0.1--0.3\%, at every memory latency we tried. Exact suppression has
already removed the traffic that was causing the serialisation, and what is left
is irreducible -- somebody really does have to be told, and the writer really
does have to wait for them. Once the filter is exact, the residue is not worth
steering.
